...
public class CoordCube
{
    ...
    static CoordCube()
    {
        try
        {
            CoordCubeBuildTables.Load();
        }
        catch (Exception)
        {
            CoordCubeBuildTables.Build();
            CoordCubeBuildTables.Save();
        }
    }

    public static void PreBuildCoordTables()
    {
        CoordCubeBuildTables.Build();
        CoordCubeBuildTables.Save();
    }
    ...
    private static class CoordCubeBuildTables
    {
                                ...
                                default:
                                            ...
                                            (N_SLICE2 * newURFtoDLF + newSlice) * 2 + newParity) == 0x0f)
                                    {
                                        setPruning(Slice_URFtoDLF_Parity_Prun,
                                            (N_SLICE2 * newURFtoDLF + newSlice) * 2 + newParity, (sbyte)(depth + 1));
                                        done++;
                                    }
                                ...
                                default:
                                            ...
                                            (N_SLICE2 * newURtoDF + newSlice) * 2 + newParity) == 0x0f)
                                    {
                                        setPruning(Slice_URtoDF_Parity_Prun,
                                            (N_SLICE2 * newURtoDF + newSlice) * 2 + newParity, (sbyte)(depth + 1));
                                        done++;
                                    }
        ...
        {
            // Movement
            twistMove = Tools.DeserializeTable("twist");
            flipMove = Tools.DeserializeTable("flip");
            FRtoBR_Move = Tools.DeserializeTable("FRtoBR");
            URFtoDLF_Move = Tools.DeserializeTable("URFtoDLF");
            URtoDF_Move = Tools.DeserializeTable("URtoDF");
            URtoUL_Move = Tools.DeserializeTable("URtoUL");
            UBtoDF_Move = Tools.DeserializeTable("UBtoDF");
            MergeURtoULandUBtoDF = Tools.DeserializeTable("MergeURtoULandUBtoDF");

            //Prune
            Slice_URFtoDLF_Parity_Prun = Tools.DeserializeSbyteArray("Slice_URFtoDLF_Parity_Prun");
            Slice_URtoDF_Parity_Prun = Tools.DeserializeSbyteArray("Slice_URtoDF_Parity_Prun");
            Slice_Twist_Prun = Tools.DeserializeSbyteArray("Slice_Twist_Prun");
            Slice_Flip_Prun = Tools.DeserializeSbyteArray("Slice_Flip_Prun");
        }
